...
impl<'test> TestCx<'test> {
    ...
    fn run_valgrind_test(&self) {
        ...
        new_config.runtool = self.config.runtool.clone().map(|mut runtool| {
            let mut tmp = new_config.valgrind_path.clone().unwrap();
            tmp.push(' ');
            tmp.push_str(&mut runtool);
            tmp
        }).or(new_config.valgrind_path.clone());
        dbg!(&new_config.runtool);
        let new_cx = TestCx {
            config: &new_config,
            ..*self
        };
